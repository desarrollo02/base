...
@Named
@ViewScoped
public class BswSessionControlador extends AbstractControllerGenerico<BswSession> {
    private static final Logger log =
            LoggerFactory.getLogger(BswSessionControlador.class);

    @Override
    public void setParametros() {
        this.clase = BswSession.class;
        this.formName = ApplicationConstant.nombreFormaSession;
        this.paginaActual = "BswSession.xhtml";

        if (acercaDe.isEmpty()) {
            getAcercaDe().add("Datos de la pantalla:");
            getAcercaDe().add("- Forma:BSSES");
            getAcercaDe().add("- Controlador: BswSessionControlador.java");
            getAcercaDe().add("- EJB: GenericoEJB.java");
            getAcercaDe().add("- JPA: BswSession.java");
        }

        if (ayuda.isEmpty()) {
            getAyuda().add("Esta es la pantalla puede ser utilizada para agregar, modificar o eliminar BswSession.");
            getAyuda().add("");
    ...
    public LazyDataModel getListaTodosActualLazy() {
        return getComponentes().getLazyListSession();
    }
